...
public class CharacterMovementController : MonoBehaviour {
    ...
    [Header("Ground Check Variables")] public GameObject groundChecker;
    ...
    private Vector3
        characterVelocity; //The value we'll be using to move our player by applying it to the Character Controller.
    ...
    private void Update() {
        ...
        if (Physics.Raycast(groundChecker.transform.position, groundChecker.transform.TransformDirection(Vector3.down),
        ...
        Debug.DrawRay(groundChecker.transform.position,
            groundChecker.transform.TransformDirection(Vector3.down) * distance, Color.yellow);
    ...
    private void HandleGroundedMovement() {
        ...
        var inputSpaceMovement = new Vector3(inputAxis.x, 0, inputAxis.y);

        //Converts the stored input coordinates to the camera's horizontal orientation in world space...
        var worldSpaceMovement = camForward * inputSpaceMovement.z + camRight * inputSpaceMovement.x;
        ...
        if (inputAxis.magnitude != 0) {
            ...
            var targetRotation = Quaternion.LookRotation(worldSpaceMovement, Vector3.up);
